...
clean: 
	@if [ -d "$(BIN_DIR)" ]; then\
		echo "$(BOLD)$(GREEN)$(BD_SYS) Removing the '$(<U>)$(BIN_DIR)$(</U>)' directory system. $(RESET)";\
...
check_modification:
	...
	@if [ $$(find $(SRC_DIR) -type f -newer $(OBJ_DIR) | wc -l) -gt 0 ]; then\
	    echo "$(BOLD)  Source files have been $(<U>)modified$(</U>).$(RESET)";\
			echo " $(shell echo $(foreach word, $(shell find $(SRC_DIR) -type f -newer $(OBJ_DIR)), "  [source] $(word)\n"))";\
	else\
